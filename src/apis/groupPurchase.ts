...
//공동구매 게시글 목록 조회
export const getGroupPurchaseList = async (params: GetGroupPurchaseListParams) => {
...
//공동구매 게시글 삭제
export const deleteGroupPurchase = async (groupOrderId: number): Promise<void> => {
...
//인기 검색어 조회
export const getGroupPurchasePopularSearch = async (): Promise<GroupOrderPopularSearch[]> => {
  const response = await axiosInstance.get<GroupOrderPopularSearch[]>("/group-orders/popular-search");
  return response.data;
};
...
export const createGroupPurchase = async (
  ...
  formData.append(
    ...
    new Blob([JSON.stringify(requestGroupOrderDto)], { type: "application/json" })
  );
...
export const addGroupPurchaseRating = async (
  ...
  await axiosInstance.post(`/group-orders/${groupOrderId}/rating/${ratingScore}`);
};
...
// 공동구매 게시글 좋아요 취소
export const unlikeGroupPurchase = async (groupOrderId: number): Promise<number> => {
  const response = await axiosInstance.patch<number>(`/group-orders/${groupOrderId}/unlike`);
  return response.data;
};

// 공동구매 게시글 모집 완료 취소
export const cancelGroupPurchaseCompletion = async (groupOrderId: number): Promise<void> => {
  await axiosInstance.patch(`/group-orders/${groupOrderId}/unCompletion`);
};

// 공동구매 게시글 좋아요 추가
export const likeGroupPurchase = async (groupOrderId: number): Promise<number> => {
  const response = await axiosInstance.patch<number>(`/group-orders/${groupOrderId}/like`);
  return response.data;
};

// 공동구매 게시글 모집 완료 처리
export const completeGroupPurchase = async (groupOrderId: number): Promise<void> => {
